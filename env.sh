...
xtech() {
    ...
    # Zmienne bazowe technologii
    export PDK=$1
    export PDK_ROOT="/foss/pdks"
    export PDKPATH="$PDK_ROOT/$PDK"

    # Zmienne środowiskowe dla innych programów z pakietu
    export KLAYOUT_PATH="$PDKPATH/libs.tech/klayout"
    export SPICE_USERINIT_DIR="$PDKPATH/libs.tech/ngspice"

    # Domyślna biblioteka komórek standardowych w zależności od PDK
    case "$PDK" in
        gf180mcu*) export STD_CELL_LIBRARY="gf180mcu_fd_sc_mcu7t5v0" ;;
        sky130*)   export STD_CELL_LIBRARY="sky130_fd_sc_hd" ;;
        sg13g2)    export STD_CELL_LIBRARY="sg13g2_stdcell" ;;
    esac

    echo "✅ Aktywna technologia: $PDK"
    echo "🔧 Wyeksportowano zmienne dla KLayout, ngspice i komórek standardowych."

    # Przygotowanie plików pod Xschem
    local XSCHEMRC="$PDKPATH/libs.tech/xschem/xschemrc"
    ...
    local MAGICRC="$PDKPATH/libs.tech/magic/$PDK.magicrc"
